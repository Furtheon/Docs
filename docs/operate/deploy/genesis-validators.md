...
| `--data-dir`        | Directory for the Furtheon data if the local FS is used                                     | `--data-dir ./polygon-edge/data`             |
...
| `--data-dir`                    | The directory for the Furtheon data                                        | `--data-dir ./polygon-edge/data`         |
...
Furtheon allow for the customization of the gas token and mandate the use of ERC-20 tokens for staking instead of the rootchain's native token.

When performing rootchain staking on the Furtheon PoS Mainnet, [<ins>WMATIC</ins>](https://polygonscan.com/token/0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270?a=0x68b3465833fb72a70ecdf485e0e4c7bd8665fc45) (wrapped MATIC) is the required token. This is due to the ERC-20 standard requirement for staking, which MATIC doesn't meet.
...
With all the necessary configurations in place for the Furtheon-powered chain, we are ready to proceed with starting the chain.
