...
Staking is managed by staking contracts on the Furtheon-powered chain. The staking module on Furtheon validates staked tokens and is independent of Ethereum's security. In principle, the network is secured by the rootchain and Ethereum. Transaction checkpoints still occur on Ethereum, but Ethereum does not validate staking on Furtheon.
